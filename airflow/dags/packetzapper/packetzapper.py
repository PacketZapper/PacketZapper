...
class PacketZapperSession(Session):
    ...
    def request(self, method, url, *args, **kwargs):
        ...
        r = super().request(method, joined_url, *args, **kwargs)
        r.raise_for_status()
        return r
...
class PacketZapperClient():
    ...
    def start_whsniff(self, host_index=None, channel=11):
        results = []
        for host in self.__hosts_generator(host_index):
            results.append(host.post('/whsniff/start', json={'channel': channel}).json())
        return results
    ...
    def start_rtl_433(self, host_index=None, channel=25):
        results = []
        for host in self.__hosts_generator(host_index):
            results.append(host.post('/rtl_433/start', json={}).json())
        return results
